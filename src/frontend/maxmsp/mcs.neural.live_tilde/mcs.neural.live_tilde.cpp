...
class mcs_live : public object<mcs_live>, public mc_operator<> {
public:
  MIN_DIGEST{"Real-time (live) neural audio synthesis (multichannel over batches and signals)"};
  ...
  MIN_RELATED{"neural.live~, mc.neural.live~, neural.tokenizer, neural.gaussianize"};


  mcs_live(const atoms &args = {});
  ~mcs_live();
  ...
  // default, synced after load. (Matrix-noise inlets are not exposed on the mcs
  ...
  message<> maxclass_setup{
      ...
      [this](const c74::min::atoms &args, const int inlet) -> c74::min::atoms {
        // make stamp
        cout << "mcs.neural.live~ " << VERSION << " - " << NN_ENGINE_NAME
...
int mcs_live::get_batches() { return m_batches; }

void model_perform(mcs_live *mc_nn_instance) {
...
void model_perform_loop(mcs_live *mc_nn_instance) {
  std::vector<float *> in_model, out_model;
...
mcs_live::mcs_live(const atoms &args)
...
bool mcs_live::has_settable_attribute(std::string attribute) {
...
void mcs_live::operator()(audio_bundle input, audio_bundle output) {
...
void mcs_live::perform(audio_bundle input, audio_bundle output) {
...
long simplemc_multichanneloutputs(c74::max::t_object *x, long index,
                                  long count) {
  minwrap<mcs_live> *ob = (minwrap<mcs_live> *)(x);
